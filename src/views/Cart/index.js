...
const Cart = props => {
	...
	const validateNatural = (e, t) => {

		console.log(e,t,e.target.value);
		if (e.target.value) {
			if (!/^\d+$/.test(e.target.value)) {
				console.log('1-call')
				alert("Decimal are not allowed")
				e.target.value = "";
			}
		}else{
			if(!/^\d+$/.test(e)){
	...
	const onInputChange = (key, val) => {
		...
		quantityClone[key] = val.target.value;
		...
		productsClone[ind].tokenQuantity = val.target.value;
	...
	const handleDoubleChange = (a, b, c) => {
		validateNatural(b, c);
		onInputChange(a, b);
	}
						...
						products.map((item, key) =>
							<tr key={key}>
								...
								<td>
									...
									<input type="number" step="1" min="0"  onChange={(val) => { handleDoubleChange(item.id.toString(), val,this) }} />
