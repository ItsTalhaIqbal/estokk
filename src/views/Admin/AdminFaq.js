...
import { actionFaqList, actionFaqGet} from "../../redux/actions/faq";
...
const AdminFaq = props => {
  ...
  const onFaqClicked = (e, index, subIndex) => {
    dispatch(actionFaqGet(index, subIndex));
    history.push('/admin/faqs/update');
  }
                ...
                <tbody>
                  {
                    faqs.map((item, key) => (
                      item.subtitle.map((subItem, index) => (
                        <tr key={`${key}-${index}`} onClick={(e) => onFaqClicked(e, item.id, subItem.id)}>
                          <td>{key + 1}</td>
                          <td>{item.title}</td>
                          <td>{subItem.smallTitle}</td>
                        </tr>
                      ))
                    ))
                  }
