...
import { Form, Input, Layout, Radio, Checkbox, Notification } from "element-react"
...
const Checkout = connect(mapStateToProps, mapDispatchToProps)(class extends Component {
		...
		callGet(`/api/getCheckoutform?ID=${this.state.form.userId}`)
			.then((response) => {
				if (response.data) {
					...
					const index = this.state.countryList.findIndex(country => country.label == response.data.country);
					if(index  != -1) {
						this.setState({ selectedCountry : this.state.countryList[index]});
					}
					this.setState({
						...this.state,
						form: {
	...
	handleSubmit(e) {
		...
		this.refs.form.validate((valid) => {
			if (!valid) {
				this.state.isFormValidated = false;
				return;
			}
	...
	onChange(key, value) {
		...
		if(key == "country") {
			...
			this.setState({
				form: Object.assign({}, this.state.form, { [key]: value.label })
			})
		}
		else {
			this.setState({
	...
	validatePhone(e, t) {
		if (/^\d+$/.test(e) == false) {
			if (e !== "") {
				alert("Symbols and Alphabets are not allowed");
				return false;
			}
		} else {
			this.onChange("phone", e)
			return true;
		}
	}
	...
	validateUsername = (event, t, str) => {
		...
		if (!regex.test(input)) {
			...
			this.onChange(str, event)
			return true;
		}
	}

	handleDoubleChange(e, t) {
		this.validatePhone(e, t);
		this.onChange.bind(t, 'phone');
	}
	handleDoubleChangeForUsername(e, t, str) {
		console.log(str);
		this.validateUsername(e, t, str);
		this.onChange.bind(t, str);
	}
								...
								<div className="grid-content">
									<div className="register-ruleForm d-font-bold" style={{ margin: 20, borderRadius: 10 }}>
											...
											<Layout.Col span="12">
												<div className="grid-content bg-purple">
													<Form.Item label={t("Checkout.FIRST NAME")} prop="firstName" style={{ margin: 15 }}>
														<Input value={this.state.form.firstName} onChange={(e) => { this.handleDoubleChangeForUsername(e, this, 'firstName') }} />
													</Form.Item>
												</div>
											</Layout.Col>
											<Layout.Col span="12">
												<div className="grid-content bg-purple-light">
													<Form.Item label={t("Checkout.LAST NAME")} prop="lastName" style={{ margin: 15 }}>
														...
														<Input value={this.state.form.lastName} onChange={(e) => { this.handleDoubleChangeForUsername(e, this, 'lastName') }} />
										...
										<Form.Item label={t("Checkout.PHONE")} prop="phone" style={{ margin: 15 }}>
											<Input value={this.state.form.phone} onChange={(e) => { this.handleDoubleChange(e, this) }} />
										</Form.Item>
									...
									<div className="grid-content"
										...
										<div className="block" style={{ marginTop: 30 }}>
											<div className="wrapper position-relative" style={{ textAlign: "right", margin: "3% 3% 3% 0" }}>
												...
												{
													this.state.form.payCard === PAYPAL_MODE && this.state.form.isAgree && this.state.totalPrice != 0 && this.state.isFormValidated &&
