...
export default function FaqsNew () {
	...
	useEffect(()=>{
		callGet("/api/getFaqsNewData").then(data1 => {
					...
					}
                {
                    Object.entries(data).map(([key,value], i) => {
						...
						if(typeof value==='object'){
							return (
								<div className="grid grid-flow-col gap-10 grid-cols-2 faqs-container">
								...
								Object.entries(value.column_1).map(([key__,value__], i__) => {
									...
									return (
												<ContentEditable
													...
													key={i__}
													className="text-white d-text-30 col-start-1 col-end-2 d-content-dark w-full p-4 "
													style={{backgroundColor:"#33454d"}}
													disabled={userType==='ADMIN'?false:true}
