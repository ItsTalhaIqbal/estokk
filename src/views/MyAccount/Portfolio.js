...
const mapDispatchToProps = { actionUserUpdate, actionGetId }
    ...
    onCheckWallet(info) {
        ...
        .then(res => {
            if(res.data.status) {
                ...
                Notification.error({
                    title: ("Failed"),
                    message: ("You can not change your wallet address\n Contact chat support or write to support@eSTOOK.immo"),
                    type: 'Warning',
                });
            }
        })
    ...
    render() {
        ...
        if (this.state.datastate == true) {
            ...
            for (let k = 0; k < product.length; k++) {
                ...
                product[k].totalInvestment = Number(product[k].assetPrice + product[k].renovationUpgrade + 
                    product[k].operatingExpenseReimbursement +product[k].vacancyReserve +product[k].administrativeFee+product[k].initMaintainanceReserve +
                    product[k].initialRenovationReserve ).toLocaleString('en-US', {
                    style: 'currency',
                    currency: 'USD',
                });
                ...
                product[k].caprate = Number(product[k].projectIRR).toFixed(2).toLocaleString();
        ...
        return (
            <>
                ...
                {/*==============================================================================*/}
                <Layout.Row style={{ backgroundColor: '#0cc1cb', padding: "3% 0 1% 2%" }}>
                    <Layout.Col span="20">
                ...
                <Layout.Row style={{ backgroundColor: '#252730' }}>
                    <Layout.Col span="22" offset={1} style={{ margin: "2% 3% 3% 3%" }}>
                        <div className="grid-content">
                            ...
                            <div className={'d-font-bold d-text-24 text-white mt-2'}>Rent will be deposited directly to this wallet on the xDai network. eSTOOK.immo pays the fees for these transactions.
                            </div>
                        </div>
                        <hr/>
                        {/* <div className="grid-content">
                        ...
                        productMap.map((row) => (
                            <div>
                                <Layout.Row style={{ backgroundColor: '#252730', marginTop: "3%" }}>
                                    <Layout.Col md="8">
                                        <Link to = {() => "/product/"+row.address1.replaceAll(" ", "-")+"-"+row.address2.replaceAll(" ", "-")}>
                                        <div className="grid-content bg-purple">
                                            <img src={row.src} alt='marketplace' style={{ width: "100%" }} onClick={this.onClick.bind(this,row.id)}/>
                                        </div>
                                        </Link>
                                    </Layout.Col>
