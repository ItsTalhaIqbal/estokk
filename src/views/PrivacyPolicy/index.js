...
export default function GeneralInformation () {
	...
	useEffect(()=>{
		callGet("/api/getPrivacyPolicyData").then(data1 => {
    ...
    return (
      <div>
			...
			<Fade>
				<div className="img-box img-box-privacy">
				<Fade bottom delay={200}>
					<div style={{ margin: "15% 12% 6% 12%" }}>
					<div className="col-md-6">
						<div className="d-white d-font-black d-text-82">
						{t("Privacy.")}
						</div>
						<div className="grid grid-flow-col w-40">
							<span className="align-self-center justify-self-end w-8"><img src="./image/privacy/phone.png"/></span> <ContentEditable 
												html={data.phone} 
						...
						<div className="grid grid-flow-col w-40">
							<span className="align-self-center justify-self-end w-8"><img src="./image/privacy/mail.png"/></span> <ContentEditable 
												html={data.email} 
												className={`text-white d-content-dark d-text-30 h-auto p-2 bg-transparent place-self-start justify-self-start w-32`}
												disabled={userType==='ADMIN'?false:true}
                    ...
                    Object.entries(data).map(([key,value], i) => {
						...
						if(typeof value==='string'){
							return	<ContentEditable
										...
										className={`text-white d-text-30 h-auto pb-4 pt-4 pl-4 pr-4  ${i===1?'':'mt-3'}`}
