...
export const actionFaqCreate = (faqData) => (dispatch) => {
  callPost('/api/admin/faq', faqData, token)
    .then((response) => {
      ...
      dispatch({
        type: ACTION_FAQ_ADD,
        payload: response.data.faqs[response.data.faqs.length - 1],
      })
    }).catch(err => {
      console.log(err);
      Notification.error({
...
export const actionFaqGet = (ID, SUBID) => (dispatch) => {
  return callGet(`/api/faq/id?ID=${ID}&subID=${SUBID}`)
    .then(function (response) {
...
export const actionFaqUpdate = (faq) => (dispatch) => {
  return callPost('/api/admin/faq/update', faq, token)
    .then((res) => {
      Notification.success({
        title: 'Success',
        message: 'FAQ data Update Success!',
        type: 'success',
      })
      return dispatch({
        type:ACTION_FAQ_UPDATE,
        payload:res,
      })
